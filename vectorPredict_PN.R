...
# Vector host prediction from phylogenetic neighborhoods
...
for (i in 1:nloops){
  ...
  #h2o.saveModel(best_gbm,path =paste("vectorPredict_PN_model",i))
  ...
  write.csv(norm_cm,file=paste("h2o_Vector_PN_CM_",i,".csv"))
...
write.csv(accs,file="Vector_PN_out.csv",row.names = F)
write.csv(test.record,file="Vector_PN_TestSets.csv",row.names = F)
write.csv(vimps,file="Vector_PN_FI.csv",row.names = T)
